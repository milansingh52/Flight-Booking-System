...
def flight(request):
    ...
    if not o_place or not d_place or not departdate or not seat:
        # Return to the index page if any field is missing with an error message
        return render(request, 'flight/index.html', {'error': 'Please fill in all required fields.'})
    ...
    try:
        ...
        # If the date format is incorrect, return to index with an error
        return render(request, 'flight/index.html', {'error': 'Invalid departure date format.'})

    return_date = None

    # Get additional parameters
    airline = request.GET.get('airline')  # Selected airline
    stops = request.GET.get('stops')
    dept_time = request.GET.get('dept_time')
    arr_time = request.GET.get('arr_time')

    # If round-trip, validate the return date
    if trip_type == '2':
        returndate = request.GET.get('ReturnDate')
        if not returndate:
            return render(request, 'index.html', {'error': 'Return date is required for a round trip.'})

        try:
            return_date = datetime.strptime(returndate, "%m/%d/%Y")
        except ValueError:
            return render(request, 'index.html', {'error': 'Invalid return date format.'})

    # Flight search logic (unchanged)
    ...
    if seat == 'economy':
        flights = Flight.objects.filter(
            ...
            origin=origin,
            destination=destination,
            airline__iexact=airline  # Case-insensitive matching
    ...
    # Render the response for round-trip
    if trip_type == '2':
    ...
    # Render the response for one-way trip
    return render(request, "flight/search.html", {
        'flights': flights,
        'airline': airline,
        'origin': origin,
