...
class Flight(models.Model):
    ...
    # previous_economy_fare = models.FloatField(null=True, blank=True)
    # previous_business_fare = models.FloatField(null=True, blank=True)
    # previous_first_fare = models.FloatField(null=True, blank=True)
    # user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='flights', null=True, blank=True)
    # track_price = models.BooleanField(default=False)


    def __str__(self):
        days = ', '.join([day.name for day in self.depart_day.all()])  # List days of the week
        return f"{self.id}: {self.origin} to {self.destination} on {days}"
